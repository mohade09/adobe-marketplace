...
if [ -n "$APP_URL" ]; then
  ...
  if [ "$DATABRICKS_AUTH_TYPE" = "profile" ]; then
    WORKSPACE_URL=$(uvx --from databricks-cli databricks workspace current --profile "$DATABRICKS_CONFIG_PROFILE" 2>/dev/null | grep -o 'https://[^/]*' || echo "https://<your-databricks-workspace>")
  else
