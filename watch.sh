...
check_auth() {
  if [ ! -z "$DATABRICKS_CONFIG_PROFILE" ]; then
    uvx --from databricks-cli databricks auth describe --profile $DATABRICKS_CONFIG_PROFILE > /dev/null 2>&1
  elif [ ! -z "$DATABRICKS_HOST" ]; then
    uvx --from databricks-cli databricks auth describe --host $DATABRICKS_HOST > /dev/null 2>&1
  else
    uvx --from databricks-cli databricks auth describe > /dev/null 2>&1
  fi
}
...
if command -v uvx >/dev/null 2>&1; then
  if ! check_auth; then
    ...
    if [ ! -z "$DATABRICKS_CONFIG_PROFILE" ]; then
      uvx --from databricks-cli databricks auth login --profile $DATABRICKS_CONFIG_PROFILE
    elif [ ! -z "$DATABRICKS_HOST" ]; then
      uvx --from databricks-cli databricks auth login --host $DATABRICKS_HOST
    else
      uvx --from databricks-cli databricks auth login
    fi
  else
