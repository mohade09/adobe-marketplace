...
test_databricks_connection() {
    ...
    if [ -n "$profile" ]; then
        if uvx --from databricks-cli databricks current-user me --profile "$profile" >/dev/null 2>&1; then
...
if [ "$skip_env" != "true" ]; then
    ...
    if [ "$auth_choice" = "1" ]; then
        ...
        echo "Running: uvx --from databricks-cli databricks current-user me"
        ...
        if uvx --from databricks-cli databricks current-user me >/dev/null 2>&1; then
            ...
            echo "DATABRICKS_HOST='$DATABRICKS_HOST' DATABRICKS_TOKEN='$DATABRICKS_TOKEN' uvx --from databricks-cli databricks current-user me"
            exit 1
        fi
        ...
        if ! test_databricks_connection "$DATABRICKS_CONFIG_PROFILE"; then
            ...
            if [[ "$configure_profile" =~ ^[Yy]$ ]]; then
                echo "Running 'uvx --from databricks-cli databricks configure --profile $DATABRICKS_CONFIG_PROFILE'..."
                uvx --from databricks-cli databricks configure --profile "$DATABRICKS_CONFIG_PROFILE"
    ...
    if [ -z "$DBA_SOURCE_CODE_PATH" ]; then
        ...
        if [ "$DATABRICKS_AUTH_TYPE" = "profile" ]; then
            DATABRICKS_USER=$(uvx --from databricks-cli databricks current-user me --profile "$DATABRICKS_CONFIG_PROFILE" --output json 2>/dev/null | grep -o '"userName":"[^"]*"' | cut -d'"' -f4)
        else
            # For PAT auth, ensure environment variables are exported
            export DATABRICKS_HOST="$DATABRICKS_HOST"
            export DATABRICKS_TOKEN="$DATABRICKS_TOKEN"
            DATABRICKS_USER=$(uvx --from databricks-cli databricks current-user me --output json 2>/dev/null | grep -o '"userName":"[^"]*"' | cut -d'"' -f4)
        fi
    ...
    if [ "$DATABRICKS_AUTH_TYPE" = "profile" ]; then
        WORKSPACE_HOST=$(uvx --from databricks-cli databricks current-user me --profile "$DATABRICKS_CONFIG_PROFILE" --output json 2>/dev/null | grep -o '"workspaceUrl":"[^"]*"' | cut -d'"' -f4)
    else
...
echo "2. Open http://localhost:5173 to view the app"
